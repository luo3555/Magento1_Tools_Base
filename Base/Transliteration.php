<?php
class SiteFactory_Transliteration_Helper_Data {
	/**
	 * 
	 */
	public function translate($string){
		$res = $this->transliteration_clean_filename($string, 'ch');
		if( preg_match("/-$/", $res)){
			$res = substr($res,0,-1);
		}
		return $res;
	 ...
	 */
	public function transliteration_clean_filename($filename, $source_langcode = NULL) {
	  if (is_array($filename)) {
	    foreach ($filename as $key => $value) {
	      $filename[$key] = $this->transliteration_clean_filename($value, $source_langcode);
	    }
	    return $filename;
	  }
	  ...
	  $filename = $this->transliteration_get($filename, '', $source_langcode);
	  // Replace whitespace.
	 ...
	 * @see language_default()
	 */
	public function transliteration_get($text, $unknown = '?', $source_langcode = NULL) {
	  return $this->_transliteration_process($text, $unknown, $source_langcode);
	}
     ...
     *   Transliterated text.
     */
    public function _transliteration_process($string, $unknown = '?', $source_langcode = NULL) {
     ...
     */
    public function _transliteration_replace($ord, $unknown = '?', $langcode = NULL) {
      static $map = array();
